...
void runStep(float *sqrtLookupTable, uint8_t ***arr, float **densities, int x, int y, int numparticles, Particle **particles, float effectRadius, float effectSquared, int frames, float dt, int phyPerGra, float gravx, float gravy, int iter) {
    ...
    for (int nonGraphic = 1; nonGraphic <= phyPerGra; nonGraphic++) {
        ...
        for (int p = 0; p < numparticles-1; p++) {
            ...
            for (int pcomp = p+1; pcomp < numparticles; pcomp++) {
                ...
                if (fabsf(distX) < 0.0001f && fabsf(distY) < 0.0001f) {
                    distX /= fabsf(distX) * 0.0001f;
                    distY /= fabsf(distY) * 0.0001f;
                }

                float inner = distX*distX + distY*distY;
                // float inner = squareLookupTable[abs(((int)(distX)*10))] + squareLookupTable[abs(((int)(distY)*10))];

                if (inner >= effectSquared || inner < 0) continue;

                // float dist = sqrt_fast(inner, sqrtLookupTable[(int)inner-1], sqrtLookupTable[(int)inner+1]);

                float dist = (sqrtLookupTable[MAX((int)(inner-1), 0)] + sqrtLookupTable[(int)(inner+1)])/2;
...
void runSimulation(float *sqrtLookupTable, uint8_t ***arr, float **densities, int x, int y, int numparticles, Particle **particles, float effectRadius, int frames, float dt, int phyPerGra, float gravx, float gravy) {
    float effectSquared = effectRadius*effectRadius;
    
    clock_t beginTraj = clock();
    runStep(sqrtLookupTable, arr, densities, x, y, numparticles, particles, effectRadius, effectSquared, frames, dt, phyPerGra, gravx, gravy, 0);
    ...
    for (int i = 1; i < frames; i++) {
        runStep(sqrtLookupTable, arr, densities, x, y, numparticles, particles, effectRadius, effectSquared, frames, dt, phyPerGra, gravx, gravy, i);
    }
}
...
int main(int argc, char** argv) {
    ...
    runSimulation(sqrtLookupTable, arr, densities, x, y, numParticle, particles, effectRadius, frames, dt, phyPerGra, gravx, gravy);
